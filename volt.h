//#ifndef ANDROID_PROJECT_MYLIB_H
//#define ANDROID_PROJECT_MYLIB_H
#include <iostream>
#include <string_view>
#include <cmath>
#include <vector>
...
namespace MYLIB {
	...
	* this is because CellBlock relies on SmartFrame alot but CellBlock must be copy constructible
	* and std::atomic's are not;
	*/
	std::mutex SmartFrameMux;

	class SmartFrameHandler;

	class SmartFrame {
	public:
		friend class SmartFrameHandler;

        auto pollEvent(SDL_Event *_event) const -> int
        {
            if (must_not_sleep_requests)return SDL_PollEvent(_event);
			else return SDL_WaitEvent(_event);
        }

		auto hasRequests(){
			if (!must_not_sleep_requests)
				return false;
			return true;
		}

    private:
		std::uint32_t must_not_sleep_requests = 0;
	};

	class SmartFrameHandler {
	public:
		void setSmartFrame(SmartFrame* _smartFrame){
			smartFrame = _smartFrame;
		}
		
		void startRedrawSession(){
			if (!redrawRequested) {
				std::scoped_lock lock(SmartFrameMux);
				smartFrame->must_not_sleep_requests++;
	...
	*/
	...
	public:
		...
		SDL_FRect min_bounds = { 0.f, 0.f, 0.f, 0.f };
		bool hidden = false;
	public:
		IView* getView() {
			return this;
		...
		template<typename T>
		constexpr inline T to_min(const T& val, const T& a, const T& b) {
			if (a < b || a == b) return toCust(val, a);
			else if (a > b) return toCust(val, b);
		}
	};
	...
	public:
		...
		void setContext(Context* _context) {
			...
			pv = _context->pv;
			smartFrame = _context->smartFrame;
	...
	public:
		...
		virtual bool handleEvent()override{
			if (smartFrame->pollEvent(event) != 0) {
				switch (event->type)
				{
				case SDL_EVENT_QUIT:
					quit = true;
	...
	void draw_ring_quadrand(SDL_Renderer* _renderer, const float& _x, const float& _y, const float& _inner_r, const float& _outer_r,
		...
		const std::chrono::steady_clock::time_point tm_start_ = std::chrono::high_resolution_clock::now();
		if (_quadrant == QUADRANT::TOP_LEFT)draw_ring_top_left_quadrant(_renderer, _x, _y, _inner_r, _outer_r, _color);
		else if (_quadrant == QUADRANT::TOP_RIGHT)draw_ring_top_right_quadrant(_renderer, _x, _y, _inner_r, _outer_r, _color);
		else if (_quadrant == QUADRANT::BOTTOM_LEFT)draw_ring_bottom_left_quadrant(_renderer, _x, _y, _inner_r, _outer_r, _color);
		else if (_quadrant == QUADRANT::BOTTOM_RIGHT)draw_ring_bottom_right_quadrand(_renderer, _x, _y, _inner_r, _outer_r, _color);
		std::chrono::duration<double> dt = (std::chrono::high_resolution_clock::now() - tm_start_);
		SDL_Log("CDF: %f", dt.count());
	}
	...
	void draw_filled_quadrant(SDL_Renderer* _renderer, const float& _x, const float& _y, const float& _r, const QUADRANT& _quadrant,
		const SDL_Color& _color = { 0xff, 0xff, 0xff, 0xff }) {
		//const std::chrono::steady_clock::time_point tm_start_ = std::chrono::high_resolution_clock::now();
		if (_quadrant == QUADRANT::TOP_LEFT)draw_filled_topleft_quadrant(_renderer, _x, _y, _r, _color);
		else if (_quadrant == QUADRANT::TOP_RIGHT)draw_filled_topright_quadrant(_renderer, _x, _y, _r, _color);
		else if (_quadrant == QUADRANT::BOTTOM_LEFT)draw_filled_bottomleft_quadrant(_renderer, _x, _y, _r, _color);
		else if (_quadrant == QUADRANT::BOTTOM_RIGHT)draw_filled_bottomright_quadrant(_renderer, _x, _y, _r, _color);
	...
	void draw_quadrant(SDL_Renderer* _renderer, const float& _x, const float& _y, const float& _r, const QUADRANT& _quadrant,
		const SDL_Color& _color = { 0xff, 0xff, 0xff, 0xff }) {
		//const std::chrono::steady_clock::time_point tm_start_ = std::chrono::high_resolution_clock::now();
		if (_quadrant == QUADRANT::TOP_LEFT)draw_topleft_quadrant(_renderer, _x, _y, _r, _color);
		else if (_quadrant == QUADRANT::TOP_RIGHT)draw_topright_quadrant(_renderer, _x, _y, _r, _color);
		else if (_quadrant == QUADRANT::BOTTOM_LEFT)draw_bottomleft_quadrant(_renderer, _x, _y, _r, _color);
		else if (_quadrant == QUADRANT::BOTTOM_RIGHT)draw_bottomright_quadrant(_renderer, _x, _y, _r, _color);
	...
	namespace Experimental {
		...
		void blurIMG(SDL_Surface* imageSurface, const int& blurExtend, const int& iterations = 1) //This manipulates with SDL_Surface and gives it box blur effect
		{
			...
			for (int i = 0; i < iterations; ++i)blurFunc(data, imageSurface->w, imageSurface->h, blurExtend);
			std::chrono::duration<double> dt = (std::chrono::high_resolution_clock::now() - start);
	...
	/**Give prog fake work to do while waiting for focus regain.
		 * Called by MYLIB::checkAppFocus() when your app enters the background.
		 * Helps to recover all your textures when your app gains focus again.
		 * */
	void safePauseApp(const int& ms) {
		SDL_Log("MYLIB::app has lost focus!");
		SDL_Event paEvent;
		bool paDone = 0;
		while (!paDone) {
			while (SDL_PollEvent(&paEvent) != 0) {
				...
				case SDL_EVENT_DID_ENTER_FOREGROUND:
					SDL_Log("MYLIB::app has gained focus!");
					paDone = 1;
					break;
				default:
					break;
	...
	public:
		...
		Interpolator& update() {
			...
			prev_dy = dy;
			//SDL_Log("%f", value);
			if (dy >= MAX_LEN - 1.f)update_physics = false;
			return *this;
		}
	...
	namespace Experimental {
		...
		std::deque<SDL_FRect> points;
            ...
            {
                if (newWidth == dest.w)return *this;
			...
			Wave& decrementWidthBy(float incVal) {
				if (dest.w - incVal < 1)return *this;
				dest.w -= incVal;
			...
			Wave& Draw() {
				if (SDL_GetTicks() - tmLsFrame > animDelay) {
					...
					std::for_each(points.begin(), points.end(), [](SDL_FRect& p) { p.x--; });
					if (points.front().x < dest.x)points.pop_front();
					t += ti;
					tmLsFrame = SDL_GetTicks();
				}
	...
	public:
		...
		FontAttributes(const std::string& a_font_file, const FontStyle& a_font_style,
			const uint8_t& a_font_size) : font_file(
				a_font_file), font_size(a_font_size), font_style(a_font_style) {}
	...
	public:
		~FontStore() {
			for (auto& [path_, font] : fonts) TTF_CloseFont(font);
		}
	...
	public:
		~U8FontStore() {
			for (auto& [font_path, font] : fonts) TTF_CloseFont(font);
		}
	...
	public:
		...
		std::optional<UniqueTexture> genTextTextureUnique(SDL_Renderer* renderer, const char* text, const SDL_Color text_color) {
			...
			if (!result.get())SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "%s", SDL_GetError());
			//SDL_Rect messageRect = {(int) x, (int) y, textSurf->w, textSurf->h};
			//SDL_DestroySurface(textSurf);
			Async::GThreadPool.enqueue([](SDL_Surface* surface) {SDL_DestroySurface(surface); surface = nullptr; }, textSurf);
			//textSurf = nullptr;
			return result;
		}
		...
		std::optional<UniqueTexture> u8GenTextTextureUnique(SDL_Renderer* renderer, const char8_t* text, const SDL_Color text_color) {
			...
			textSurf = nullptr;
			if (!result.get()) {
				SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "%s", SDL_GetError()); return {};
			}
			return result;
		}
	...
	public:
		...
		TextView& updateTextColor(const SDL_Color& bgColor, const SDL_Color& textColor) {
			...
			auto genTex = FontSystem::Get().setFontAttributes(fontAttributes)
				.genTextTextureUnique(renderer, textAttributes.text.c_str(), textAttributes.text_color);
			/*int tmp_w = 0, tmp_h = 0;
		...
		TextView& Build( const TextViewAttributes& tvAttr) {
			...
			if (genTex.has_value()) {
				...
				if (tmp_h < rect.h)rect.y = (tvAttr.rect.h - tmp_h) / 2.f, rect.h = tmp_h;
				...
				if (!shrinkToFit)rect.x = tvAttr.rect.x, rect.w = tvAttr.rect.w;
				else rect.x += tvAttr.rect.x;
	...
	public:
		...
		TextSlider& Build(const SDL_FRect& dest_, const TextAttributes& text_attributes, const FontAttributes& font_attributes) {
			animSwitchSmartFrameHD.setSmartFrame(smartFrame);
			slideSmartFrameHD.setSmartFrame(smartFrame);
			if (!contextIsSet) {
				SDL_Log("Couldn't create TextSlider: Context is null/wasn't set");
				return *this;
			}

			if (sliderTexture.get() != nullptr) sliderTexture.reset();
		...
		TextSlider& updateText(const char* newText) {
			...
			slide = true;
			if (oldTexture.get() != nullptr) oldTexture.reset();
			...
			*/
			...
			animateSwitch = true;
			rTargetCache.release(renderer);
			if (animateSwitch)animSwitchSmartFrameHD.startRedrawSession();
			return *this;
		}

		const TextSlider& Draw() {
			if (animateSwitch) {
				if (animDestA.y <= -animDestA.h)animateSwitch = false, animSwitchSmartFrameHD.stopRedrawSession(), animDestB.y = newDest.y, tm_start = SDL_GetTicks(),tm_last = SDL_GetTicks();
				CacheRenderTarget rTargetCache(renderer);
				SDL_SetRenderTarget(renderer, sliderTexture.get());
				renderClear(renderer, textAttributes.bg_color.r, textAttributes.bg_color.g, textAttributes.bg_color.b,
					textAttributes.bg_color.a);
				...
				rTargetCache.release(renderer);
				animDestA.y -= vi;
				if (animDestB.y > newDest.y)animDestB.y -= vi;
			}
			else {
				if (slide) {
					//updateSlide();
	...
	private:
		...
		void updateTextures() {
			...
			if (!textAttributes.text.empty()) {
				auto textTex=FontSystem::Get().setFontAttributes(fontAttributes)
					.genTextTextureUnique(renderer, textAttributes.text.c_str(), textAttributes.text_color);
				int tmp_w = 0, tmp_h = 0;
				SDL_QueryTexture(textTex.value().get(), nullptr, nullptr, &tmp_w, &tmp_h);
				newTw = (float)tmp_w, newTh = (float)tmp_h;
				if (newTexture.get() != nullptr) newTexture.reset();
				...
				if (newTw < newDest.w) newDest.x = newDest.x + ((dest.w - newTw) / 2.f), newDest.w = newTw, slide = false;
				if (newTh < newDest.h) {
			...
			tm_start = SDL_GetTicks();
			tm_last = SDL_GetTicks();
			if (slide)slideSmartFrameHD.startRedrawSession();
			else slideSmartFrameHD.stopRedrawSession();
		}
		...
		void updateSlide() {
			...
			//vel = 1;
			source_a.x += vel;
			if (source_a.x + source_a.w > newTw) newDest.w -= vel;
	...
	private:
		...
		int vel = 1;
		SmartFrameHandler animSwitchSmartFrameHD, slideSmartFrameHD;
	};
		...
		public:
			...
			RunningText& Build(Context* _context, const RunningTextAttributes& _attr) {
				setContext(_context);
				smartFrameHD.setSmartFrame(smartFrame);
				attr = _attr;
				bounds = _attr.rect;
			...
			void updateText(const std::string& _text) {
				...
				if (static_cast<float>(th) > dst.h)dst.y += fd_, dst.h = static_cast<float>(th);
				if (dst.w < bounds.w)is_centered = true, dst.x += ((bounds.w - dst.w) / 2.f);
				...
				smartFrameHD.startRedrawSession();
			}
		...
		private:
			...
			SmartFrameHandler smartFrameHD;
		};
	...
	public:
			...
			if (event.type == SDL_EVENT_FINGER_DOWN) {
				...
				if (found && tmpSelectedTextViewIndex != selectedTextViewIndex) {
					...
					if (is_scrollable) {
						...
						if (distance_to_move < 0.f)ACTN_MINUS = true;
						else ACTN_MINUS = false;
		...
		void udpdateSelectedText(const uint32_t& _index) {
			...
			if (_index != selectedTextViewIndex) {
				if (onSelectedTextViewUpdateCallback != nullptr)onSelectedTextViewUpdateCallback(textViews[selectedTextViewIndex]);
				textViews[selectedTextViewIndex].updateTextColor({ 0, 0, 0, 0 }, unselectedTextColor);
		...
		TextNavBar& Draw() {
			if (UPDATE_SELECTED_TEXTVIEW) {
				...
				for (auto& textview : textViews) {
					if (UPDATE_SELECTED_TEXTVIEW)textview.rect.x += dy;
					textview.Draw();
				}
				dy = 0.f;
				rTargetCache.release(renderer);
		...
		TextNavBar& Build(const std::vector<std::string>& _texts, const std::string& _fontFile,
			...
			for (const auto& text : _texts) {
				...
				textViews.back().setContext(getContext())
					.setId(i)
					.Build({
				.rect={ (i * spacing) + prev_tv_width, 0, rect.w/*to_cust(40.f, _dest.w)*/, rect.h },
				.textAttributes={ text, i == _defaultSelectedTextIndex ? selectedTextColor : unselectedTextColor, {0,0,0,0} },
				.fontFile=_fontFile,
			...
			if (textViews.back().rect.x + textViews.back().rect.w > rect.w) {
				...
				std::for_each(textViews.begin(), textViews.end(), [&dd](TextView& textView) { textView.rect.x += dd; });
				is_scrollable = true;
			}
			else {
				float dx = rect.w - (textViews.back().rect.x + textViews.back().rect.w);
				dx /= 2.f;
				std::for_each(textViews.begin(), textViews.end(), [&dx](TextView& textView) { textView.rect.x += dx; });
			}
	...
	public:
		...
		void Draw() const {
			if (inner_color.a < 1 && border_color.a < 1)return;
	...
	public:
		...
		void Build(const float& x_centre, const float& y_centre, const float& r,
			const SDL_Color& col, const bool& filled = true,
			const unsigned short& quadrant = 0) {
			if (quadrant == 0)dest = { r, r, r + r, r + r };
			else if (quadrant == 1)dest = { x_centre - r, y_centre - r, r, r };
			else if (quadrant == 2)dest = { x_centre, y_centre - r, r, r };
			else if (quadrant == 3)dest = { x_centre - r, y_centre, r, r };
			else if (quadrant == 4)dest = { x_centre, y_centre, r, r };
	...
	public:
		...
		*/
		...
		void Draw()override
		{
			[[unlikely]] if (build_with_async_) checkAndProcessAsyncProgress();
		...
		bool handleEvent()override
		{
			...
			if (event->type == SDL_EVENT_MOUSE_BUTTON_DOWN) {
				...
				if (isPointInBound(event->button.x, event->button.y) && touch_down_/* && !motion_occured_*/) {
					//SDL_Log("released");
					if (onClickedCallBack_ != nullptr)onClickedCallBack_();
					result = true;
				}
				touch_down_ = false;
	...
	private:
		...
		bool is_async_load_done()const noexcept{
			...
			if(!build_with_async_)return true;
			if (/*async_load_future_._Is_ready()*/async_load_future_.wait_for(std::chrono::seconds(0)) == std::future_status::ready)return true;
			return false;
		}

		void checkAndProcessAsyncProgress()noexcept {
			if (is_async_load_done()) {
				smartFrameHD.stopRedrawSession();
		...
		SDL_Surface* async_img_Load(const std::string& _path)noexcept {
			SDL_Surface* surface;
			if (!(surface = IMG_Load(_path.c_str())))SDL_Log("%s", IMG_GetError());
			//SDL_Delay(10000);
			return surface;
		}
	...
	public:
		...
		void Draw() {
			...
			if (diff_ <= m_blink_tm)
				...
				RestoreCachedRenderColor(renderer);
			else if (diff_ >= m_blink_tm * 2)m_start = SDL_GetTicks();
		}
	...
	public:
		...
		TextBox& Build(Context* _context, const TextBoxAttributes& textboxAttr_) {
			...
			if (text_attributes_.text.size() < max_displayable_chars_per_ln_)wrapped_text_.emplace_back(text_attributes_.text);
			else {
	...
	public:
		...
		bool handleEvent() {
			...
			case SDL_EVENT_MOUSE_BUTTON_DOWN:
				if (onClick(event->button.x, event->button.y))mouse_in_bound_ = true;
				else mouse_in_bound_ = false;
				break;
		...
		void genText() {
			...
			for (auto const& line_ : wrapped_text_) {
				...
				if (textTex.has_value())
				{
					...
					//text_rect_.w = std::clamp(text_rect_.w, 0.f, cache_text_rect.w);
					if (static_cast<float>(tmp_sh) > text_rect_.h)text_rect_.y += fd_, text_rect_.h = static_cast<float>(tmp_sh);
	...
	public:
		...
		TextBoxNavBar& handleEvent(const SDL_Event& event) {
			if (event.type == SDL_EVENT_FINGER_DOWN) {
				...
				const SDL_FPoint cf = { (event.tfinger.x * DisplayInfo::Get().RenderW) - rect.x,
									   (event.tfinger.y * DisplayInfo::Get().RenderH) - rect.y };
				std::size_t tmpSelectedTextViewIndex;
				const bool found = std::any_of(textViews.begin(), textViews.end(), [&](const MYLIB::TextBox& textView) {
		...
		void udpdateSelectedText(const uint32_t& _index) {
			...
			if (_index != selectedTextViewIndex) {
				...
				//SDL_Log("DM: %f", distanceMoved);
				std::for_each(textViews.begin(), textViews.end(), [&](MYLIB::TextBox& textView) {
		...
		TextBoxNavBar& Draw() {
			if (UPDATE_SELECTED_TEXTVIEW) {
				...
				renderClear(renderer, bgColor.r, bgColor.g, bgColor.b, bgColor.a);
				for (auto& textview : textViews) {
					if (UPDATE_SELECTED_TEXTVIEW)textview.updatePosBy(dy, 0.f);
					textview.Draw();
				}
				dy = 0.f;
				rTargetCache.release(renderer);
	...
	private:
		TextBoxNavBar& registerOnSelectedTextViewUpdateCallback(std::function<void(const MYLIB::TextBox&)> _onSelectedTextViewUpdateCallback) {
			onSelectedTextViewUpdateCallback = std::move(_onSelectedTextViewUpdateCallback);
			return *this;
		}

		TextBoxNavBar& Build(const std::vector<std::string>& _texts, MYLIB::TextBoxAttributes _tbcd, const size_t& _defaultSelectedTextIndex = 0) {
			//if (_dest.w < 1 || _dest.h < 1)return *this;
			//Context::setContext(_context);
			/*rect = _dest;
			bgColor = _bgColor;
			...
			if (textViews.back().getBounds().x + textViews.back().getBounds().w > rect.w) {
				float dd = get_distance_to_move(_defaultSelectedTextIndex);
				std::for_each(textViews.begin(), textViews.end(), [&dd](MYLIB::TextBox& textView) { textView.updatePosBy(dd, 0.f); });
				is_scrollable = true;
			}
			else {
				float dx = rect.w - (textViews.back().getBounds().x + textViews.back().getBounds().w);
				dx /= 2.f;
				std::for_each(textViews.begin(), textViews.end(), [&dx](MYLIB::TextBox& textView) { textView.updatePosBy(dx, 0.f); });
			}
			selectedTextViewIndex = std::clamp(_defaultSelectedTextIndex, std::size_t(0), _texts.size());
	...
	private:
		...
		std::vector<MYLIB::TextBox> textViews;
		std::shared_ptr<SDL_Texture> texture;
		std::function<void(const MYLIB::TextBox&)> onSelectedTextViewUpdateCallback = nullptr;
		bool is_scrollable = false;
	};
	...
	public:
		...
		TextBoxNavBarBuilder& setTextBoxConfigData(MYLIB::TextBoxAttributes _tbcd) {
			tbcd = _tbcd;
			return *this;
		}
	...
	public:
		...
		void Draw()override {
			...
			if (not hasFocus) {
				...
				else SDL_RenderTexture(renderer, txtTexture.get(), nullptr, &finalTextRect);
			}
			else {
		...
		bool handleEvent()override {
			bool result_ = false;
			if (hasFocus and not SDL_TextInputActive())SDL_StartTextInput();
			switch (event->type) {
			case SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED:
				{
			...
			case SDL_EVENT_FINGER_DOWN:
			{
				if (onClick(event->tfinger.x * DisplayInfo::Get().RenderW, event->tfinger.y * DisplayInfo::Get().RenderH) and not hasFocus) {
					...
					if (hasFocus) {
						SDL_StopTextInput(), hasFocus = false, smartFrameHD.stopRedrawSession();
						if (highlightOnHover) {
							outlineRect.outline_color = outlineColor;
				...
				if (hasFocus) {
					...
					if (event->key.keysym.scancode == SDL_SCANCODE_BACKSPACE and not internalText.empty()) {
						internalText.erase(internalText.size() - (inputSize.back())),
							inputSize.pop_back();
						updateTextTexture("");
						if (onTextInputCallback)onTextInputCallback(*this);
			...
			case SDL_EVENT_TEXT_INPUT:
			{
				if (hasFocus) {
					...
					if (onTextInputFilterCallback)onTextInputFilterCallback(*this, newText);
	...
	protected:
		void updateTextTexture(const std::string& _newText) {
			...
			CacheRenderTarget rTargetCache(renderer);
			txtTexture.reset();
			auto textTex=FontSystem::Get().setFontAttributes(fontAttributes, 0)
				.genTextTextureUnique(renderer, visibleText.c_str(), textAttributes.text_color);
			int tmpW, tmpH;
	...
	protected:
		...
		SmartFrameHandler smartFrameHD, updSmartFrameHD;
		bool highlightOnHover = false;
		SDL_Color onHoverOutlineColor = { 0x00, 0x00,0x00,0x00 };
		SDL_Color onHoverBgColor = { 0x00, 0x00,0x00,0x00 };
		SDL_Color onHoverTxtColor = { 0x00, 0x00,0x00,0x00 };
	...
	namespace Expr {
		...
		public:
			...
			bool handleEvent()override {
				...
				case SDL_EVENT_MOUSE_BUTTON_DOWN:
					if (pointInBound(event->motion.x, event->motion.y)) {
						...
						if (Orientation::VERTICAL == orientation)updateValue(screenToWorld(event->motion.y - rect.y));
						else if (Orientation::HORIZONTAL == orientation)updateValue(screenToWorld(event->motion.x - rect.x));
					}
					break;
				case SDL_EVENT_MOUSE_MOTION:
					if (key_down) {
			...
			auto getCurrentValue() {
				if (Orientation::VERTICAL == orientation)return max_val - (current_val - min_pad);
			...
			Slider& updateValue(float _val) {
				...
				if (onValUpdate)onValUpdate(*this);
				return *this;
			}
			...
			Slider& updateMinMaxValues(float _min_val, float _max_val, float _current_val) {
				...
				if (min_val < 0.f)min_val = fabs(min_val), min_pad = min_val * 2.f;
				if (max_val < 0.f)max_val = fabs(max_val), max_pad = max_val * 2.f;
				max_val += min_pad;
	...
	public:
		...
		Slider& SetUp(Context* _context, SliderAttributes _attr){
			...
			orientation = _attr.orientation;
			w = _attr.w;
			if (w == 0.f)w = DisplayInfo::Get().pw(1.5f);
			if (radius == 0.f)radius = w;
			radius += 1;
			x = _attr.x, y = _attr.y;
			length = _attr.length;
		...
		Slider& Build(float _min_val, float _max_val) {
			...
			if (min_val >= 0)dt_min_max = max_val - min_val;
			else dt_min_max = max_val + fabs(min_val);
		...
		Slider& UpdateValue(const float& value) {
			if (value < min_val)current_val = min_val, SDL_Log("slider value is less than min_val");
		...
		bool handle_event(const SDL_Event& e) {
			...
			case SDL_EVENT_MOUSE_BUTTON_DOWN:
				if (OnClick(e.motion.x, e.motion.y)) {
					key_down = true, RESULT = true;
					if (orientation == Orientation::VERTICAL)update(length - (e.motion.y - y));
					else if (orientation == Orientation::HORIZONTAL)update(e.motion.x - x);
				}
				break;
			case SDL_EVENT_MOUSE_MOTION:
				if (key_down) {
		...
		void Draw() const {
			if (ready) {
				...
				if (show_knob)knob.Draw(renderer);
			}
		}
	...
	private:
		...
		void update(const float& val) {
			if (orientation == Orientation::VERTICAL) {
				current_bar_val = val;
				if (current_bar_val > length)current_bar_val = length;
	...
	public:
		...
		bool HandleEvent() {
			...
			switch (event->type) {
			case SDL_EVENT_MOUSE_BUTTON_DOWN:
				if (OnClick(event->motion.x, event->motion.y))clicked = true;
				break;
			case SDL_EVENT_MOUSE_MOTION:
				if (!OnClick(event->motion.x, event->motion.y))clicked = false;
				break;
			case SDL_EVENT_MOUSE_BUTTON_UP:
				if (OnClick(event->motion.x, event->motion.y) && clicked) {
					if (checked)checked = false, this->knob.dest.x = this->knob_x_unchecked;
					else checked = true, this->knob.dest.x = knob_x_checked;
					RESULT = true;
					if (onClickedCallback != nullptr)onClickedCallback();
				}
				this->clicked = false;
				break;
			default:
	...
	public:
		...
		void set_range_max(const float& range_max) {
			...
			else if (range_max == range_max_)return;
	...
	public:
		...
		inline void draw() const {
			if (texture.get() != nullptr)SDL_RenderTexture(renderer, texture.get(), nullptr, &dest);
		}
	...
	private:
		...
		public:
			...
			*/
			...
			Cell& addImageButton(T& parent_block, ImageButtonAttributes imageButtonAttributes, const PixelSystem& pixel_system= PixelSystem::PERCENTAGE){
				...
				if (imageButtonAttributes.image_load_style == IMAGE_LD_STYLE::ASYNC_PATH || imageButtonAttributes.image_load_style == IMAGE_LD_STYLE::ASYNC_CUSTOM_SURFACE_LOADER)parent_block.addToCellsWithAsyncImage(&imageButton.back());
				return *this;
			}
			...
			Cell& addSlider(Expr::SliderAttributes sAttr){
				sAttr.rect = { bounds.x+pv->to_cust(sAttr.rect.x,bounds.w),
			...
			bool handleEvent()override{
				...
				if (customEventHandlerCallback != nullptr)
					return customEventHandlerCallback(*this);
				if (event->type == SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED) {
						bounds =
						{
							DisplayInfo::Get().toUpdatedWidth(bounds.x),
		...
		class CellBlock : public Context, IView {
		public:
			using IView::bounds;
			using IView::getView;
			using IView::hidden;
		...
		public:
			...
			CellBlock& updateHeight(const float& height) {
				if (height == bounds.h)return *this;
				const float df = height - bounds.h;
				if (df >= 0.f) dy = df, ACTION_UP = true;
				else dy = 0.f, ACTION_DOWN = true;
				this->bounds.h = height;
				this->margin.h += df;
				if (texture.get() != nullptr) texture.reset();
				CacheRenderTarget rTargetCache(renderer);
			...
			CellBlock& clearAndReset() {
				...
				for (auto preAddedCellSetUpCallback : preAddedCellsSetUpCallbacks) {
					...
					///FIXME: the cells smartFrame =&CellsSmartFrame should be a single
					///call with the cell.setContext so that any cell::smartframehd can be init
					///with the correct SmartFrame cotext
					cells.back().smartFrame = &CellsSmartFrame;
			...
			CellBlock& addCell(std::function<void(Cell&)> newCellSetUpCallback){
				if(!BuildWasCalled){
					...
					auto& new_cell=cells.emplace_back()
						.setContext(this)
						.setIndex(cells.size() - 1);
					new_cell.smartFrame = &CellsSmartFrame;
			...
			*/
			CellBlock& setCellRect(Cell& _cell, const uint32_t& numVerticalModules, const float& h, const float& margin_x = 0.f, const float& margin_y = 0.f) {
				if (getMaxVerticalCells() - consumedCells < numVerticalModules)lineCount++, _cell.bounds.y += margin_y;
				...
				if (cells.size() > 1) {
					...
					if (lbc.y + lbc.h < bck.y + bck.h)lbc = bck;
				}

				_cell.bounds.y += lbc.y;
				if (lineCount != prevLineCount) consumedCells = 0, _cell.bounds.y += lbc.h + CellSpacingY;
			...
			[[nodiscard]] bool isBlockScrollable() const{
				...
				if (cells.back().bounds.y + cells.back().bounds.h > margin.h && cells.front().bounds.y < 0.f) return true;
				return false;
			}

			bool handleEvent()override {
				bool result = false;
				if (not enabled)return result;// *this;
				if (event->type == SDL_EVENT_RENDER_TARGETS_RESET) {
					...
					if (not KEYDOWN) {
						...
						if (isPosInbound(event->motion.x, event->motion.y)) {
							...
							if (not cellFound) {
								if (HIGHLIGHTED_CELL >= 0) SIMPLE_RE_DRAW = true;
								updateHighlightedCell(-1);
							}
						}
						else {
					...
					if (MOTION_OCCURED and isPosInbound(cf.x, cf.y) and isBlockScrollable()) {
						...
						if (finger_up_dt > 45)dt = 5000.f, movedDistance.y = 0.1f;
						if (dt == 0.f)dt = 0.0001f;
						auto speed = (SDL_fabsf(movedDistance.y) / dt) * 10.f;
						//SDL_Log("Speed: %f", speed);
						if (finger_up_dt > 45)speed /= 10.f;
						//SDL_Log("DS_MOVED: %f REL_TM: %d INIT_VEL: %f", fabs(moved_distance.y), dt, speed);
						if (movedDistance.y >= 0.f)ANIM_ACTION_DN = true;
						else ANIM_ACTION_UP = true;
				...
				if(CELL_PRESSED or SIMPLE_RE_DRAW or scrollAnimInterpolator.isAnimating() or CellsSmartFrame.hasRequests()){
					smartFrameHD.startRedrawSession();
				}
				return result;// *this;
			}
			...
			void scroll(float distance){
				if (distance >= 0.f)ANIM_ACTION_DN = true;
				else ANIM_ACTION_UP = true;
			...
			void Draw()override {
				...
				if (SIMPLE_RE_DRAW) { SIMPLE_RE_DRAW = false; goto simple_re_draw; }
				if (not ACTION_DOWN and not ACTION_UP and not ANIM_ACTION_DN and not ANIM_ACTION_UP and not CELL_PRESSED and not smartFrameHD.shouldReDrawFrame()) {
				...
				if (scrollAnimInterpolator.isAnimating()) {
					...
					if (ANIM_ACTION_DN) {
						...
						if (cells.back().bounds.y+cells.back().bounds.h + dy < margin.h and
							...
							cells.back().bounds.h, scrollAnimInterpolator.setIsAnimating(false), ACTION_UP = false, ACTION_DOWN = false;
					}
				}
				else ANIM_ACTION_DN = ANIM_ACTION_UP = false;
				...
				if (not cells.empty()) {
					if (dy not_eq 0.f) {
						...
						std::for_each(cells.begin(), cells.end(), [this](Cell& cell) { cell.updatePosBy(0.f, dy); });
					}
					if (CELL_PRESSED) {
						CELL_PRESSED = false;
					...
					if (cells.back().index >= maxCells - 1)FIRST_LOAD = false;
					...
					if (ACTION_UP) {
						...
						while (visibleCells.back()->bounds.y < margin.h and
							...
							if (FIRST_LOAD and (cells.back().index == visibleCells.back()->index) and cells.back().index < (maxCells - 1)) {
								cells.emplace_back();
								cells.back().setContext(this)
									.setIndex(cells[cells.size() - 2].index + 1)
									.bg_color = cell_bg_color;
								cells.back().smartFrame = &CellsSmartFrame;
								cells.back().pv = this;
								fillNewCellDataCallback(cells.back());
						...
						while (visibleCells[0]->bounds.y + visibleCells[0]->bounds.h < 0.f)visibleCells.pop_front(), frontIndex = visibleCells.front()->index;
					}
					...
					if (ACTION_DOWN) {
						...
						while (visibleCells.back()->bounds.y > margin.h)visibleCells.pop_back(), backIndex = visibleCells.back()->index, update_top_and_bottom_cells();/*, SDL_Log("back pop rs: %d", cells_tmp.size())*/
			...
			simple_re_draw:
				...
				SDL_RenderTexture(renderer, texture.get(), nullptr, &margin);
				smartFrameHD.stopRedrawSession();
				if(CellsSmartFrame.hasRequests() or scrollAnimInterpolator.isAnimating()) {
					smartFrameHD.startRedrawSession();
				}
		...
		protected:
			void update_cells_with_async_images() {
				if (cellsWithAsyncImages.empty())return;
			...
			void update_top_and_bottom_cells()noexcept
			{
				...
				std::for_each(visibleCells.begin(), visibleCells.end(),
					[&smallest, &tmp_smallest, &tmp_top_cell, &largest, &tmp_largest, &tmp_bottom_cell](const Cell* cell)
					{
						...
						if (tmp_smallest < smallest)smallest = tmp_smallest, tmp_top_cell = cell->index;
						if (tmp_largest >= largest)largest = tmp_largest, tmp_bottom_cell = cell->index;
						tmp_smallest = 0.f;
						tmp_largest = 0.f;
					});
			...
			void updateSelectedCell(int64_t _selectedCell) {
				...
				if (PREV_SELECTED_CELL >= 0)cells[PREV_SELECTED_CELL].selected = false;
				cells[SELECTED_CELL].selected = true;
			}

			void updateHighlightedCell(int64_t _highlightedCell) {
				//clear the prev highlighted cell
				if (HIGHLIGHTED_CELL >= 0)cells[HIGHLIGHTED_CELL].isHighlighted = false;
				...
				if (HIGHLIGHTED_CELL >= 0)cells[HIGHLIGHTED_CELL].isHighlighted = true;
			}
			...
			void internal_handle_motion() {
				...
				if (cf.y < pf.y) {
					if (cells.back().bounds.y + cells.back().bounds.h - (pf.y - cf.y) <
						margin.h && cells.back().index == maxCells - 1) {
						dy += 0.f;
					}
